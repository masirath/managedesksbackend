...
const update_purchase_order = async (req, res) => {
  ...
  try {
    ...
    for (const detail of details) {
      ...
      let total_quantity =
        parseFloat(detail?.quantity || 0) / parseFloat(detail?.conversion || 0);

      // total stock calculation for existing stock
      let total_delivered = detail.delivered;
      let current_stock = 0;
      if (detail.inventory) {
        const selected_purchase_order = await purchase_orders_details?.findOne({
        ...
        current_stock =
          ...
          parseFloat(selected_purchase_order?.delivered || 0);
        total_delivered =
        ...
        ? // update inventory
          {
            ...
            stock: isMainUnit ? total_delivered : total_quantity,
        ...
        : // create inventory
          {
            ...
            sale_price: isMainUnit ? detail?.sale_price : 0,
            tax: isMainUnit ? detail?.tax : 0,
            stock: isMainUnit ? detail.delivered : total_quantity,
            expiry_date: isMainUnit ? detail?.expiry_date : "",
            barcode: isMainUnit ? detail?.barcode : "",
